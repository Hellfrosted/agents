...
def configured_root(args: argparse.Namespace) -> Path | None:
    raw_root = args.root or os.environ.get("CODEX_WSL_WORKTREE_ROOT")
...
def build_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    external_root = configured_root(args)
    root = external_root or default_root(repo_root)
    same_project = external_root is None

    if external_root is not None and is_wsl() and str(root).startswith("/mnt/") and not args.allow_windows_mount:
        raise ValueError(
    ...
    for candidate in candidates:
        ...
        path = root / run_id / candidate_id if same_project else root / repo_root.name / run_id / candidate_id
        thread_project_id = str(repo_root if same_project else path)
        worktrees.append(
            {
                "candidate": candidate,
    ...
    return {
        ...
        "local_exclude": ".codex-worktrees/" if same_project else "",
        "worktrees": worktrees,
    }
...
def main() -> int:
    ...
    parser.add_argument("--root", help="Manual worktree root; defaults to <repo>/.codex-worktrees/architecture-cascade")
