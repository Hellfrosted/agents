...
- Safety rule: candidate edits happen in isolated worktree-backed threads after each thread confirms files, tests, conflicts, and ownership. In WSL, use same-project manual git worktrees nested under the saved project checkout unless the user explicitly chooses managed worktrees or an external root.

## Automation Upgrade

Run weekly in a coordinator worktree. Create candidate worktree-backed threads for bounded candidates; in WSL, place manual git worktrees under `<repo>/.codex-worktrees/architecture-cascade` by default. Report blocked candidates with the exact decision needed.
