...
The Go `sk-up` port may intentionally break current command behavior before the
tool is promoted to real users. This is the last low-cost window to replace
PowerShell-era workflow compromises with a coherent composable CLI contract;
after promotion, compatibility should be treated as a user-facing commitment.
There is no planned throwaway v1: the first promoted release is the
compatibility contract users will build on.
...
The existing `-z` muscle memory should remain, but it becomes an alias for
opening diffs with a configured diff tool rather than a Zed-only contract. Zed
is the default when available, and users can set `--diff-tool` or
`SK_UP_DIFF_TOOL` for other editors.
