...
Before creating candidate worktree threads, read:

- `improve-codebase-architecture` skill instructions.
- Codex Worktree/thread docs or local app thread tooling docs.
- `agent-loop-designer/references/worktree-threads.md`, especially WSL Manual Worktree Mode.
- Repo `AGENTS.md`, `CONTEXT.md`, `docs/adr/`, README/docs, and test/build guidance.

## Loop

1. Run one discovery pass using the `improve-codebase-architecture` process: domain vocabulary, ADRs, relevant docs, source, tests, and an HTML report. Apply the Report Location Guardrails before opening or sharing the report path.
2. Extract every candidate into a queue with id, title, files, problem, solution, benefits, recommendation strength, ADR conflicts, likely tests, and proposed owner.
3. Create one worktree-backed candidate thread per candidate, capped to a practical batch size. Each thread receives the candidate packet, repo guidance, docs checked, and an instruction to first verify actability before editing. Before creating any candidate thread, apply the Worktree Placement and Starting State Guardrails below.
...
6. The coordinator collects thread results. If a candidate fails verification, stop that candidate, keep other independent worktree threads moving, and report the failure with evidence.
...
Use WSL Manual Worktree Mode when the coordinator is running in WSL and either
the repository or Codex-managed worktree root is on a Windows mount such as
`/mnt/c` or `/mnt/e`.
...
- Create manual Git worktrees under a Linux-native root, defaulting to `$HOME/codex-worktrees/<repo-name>/<run-id>/<candidate-id>`.
- Use `git worktree add --detach <manual-path> <verified-ref>` unless a branch is intentionally required by the repo workflow.
- Start candidate threads as local project threads pointed at the manual worktree path, not as app-managed Worktree threads.
- If the thread API rejects the manual path because it is not a saved project, stop after creating the worktrees and ask the user to add/open those WSL worktree paths as projects. Do not fall back to Windows-backed managed worktrees without explicit user approval.
- Use Codex-managed Worktree threads only when not in WSL, when the worktree root is known to be Linux-native, or when the user explicitly chooses managed worktrees.
...
- In WSL Manual Worktree Mode, use the same verified ref in `git worktree add --detach`; do not use app `startingState`.
...
- Worktree: one isolated manual WSL git worktree or Codex-managed Worktree thread per candidate, chosen by the Worktree Placement Guardrails.
...
- Owns discovery, queue extraction, thread creation, status collection, and final summary.
